...
public class PostRepositoryTest {
	...
	public void testSave() {
		...
		Post post = new Post(PostRepository.NEW_POST_INDEX, user, "테스트", new Date());
		postRepository.savePost(post);
		List<Post> posts = postRepository.getPosts(user.getId());
		
		assertEquals(1, postRepository.getLastPostIndex());
		assertEquals("테스트", posts.get(0).getContent());
	}
	...
	public void testDelete() {
		User user = new User("jungmin", "1234", "김정민", "ssub", "c:\\fucku");
		Post post = new Post(PostRepository.NEW_POST_INDEX, user, "테스트" +
				"", new Date());
		postRepository.savePost(post);
